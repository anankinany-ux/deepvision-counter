...
class DeepVisionCounter:
    ...
    def create_ui(self):
        ...
        tk.Label(left_frame, text="DEEP VISION", bg=self.colors['surface'], 
                fg=self.colors['accent'], font=('SF Pro Display', 24, 'bold')).pack()
        ...
        # Button row 1
        btn_row1 = tk.Frame(controls_frame, bg=self.colors['surface'])
        btn_row1.pack()
        
        self.start_btn = self.create_button(btn_row1, self.get_text('start'), 
                                                   self.start_counting, self.colors['success'])
        self.start_btn.pack(side=tk.LEFT, padx=5)
        
        self.stop_btn = self.create_button(btn_row1, self.get_text('stop'), 
                                                  self.stop_counting, self.colors['danger'])
        self.stop_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn.config(state='disabled')
        
        reset_btn = self.create_button(btn_row1, self.get_text('reset'), 
                                              self.reset_counts, self.colors['warning'])
        reset_btn.pack(side=tk.LEFT, padx=5)
        
        # Button row 2
        btn_row2 = tk.Frame(controls_frame, bg=self.colors['surface'])
        btn_row2.pack(pady=(8, 0))
        
        settings_btn = self.create_button(btn_row2, self.get_text('settings'), 
                                                 self.show_settings, self.colors['accent'])
        settings_btn.pack(side=tk.LEFT, padx=5)
        
        # Language toggle button
        lang_btn = self.create_button(btn_row2, 
        ...
        tk.Label(video_header, text=self.get_text('live_feed'), 
                ...
                font=('SF Pro Display', 16, 'bold')).pack(side=tk.LEFT, padx=20, pady=15)
        ...
        tk.Label(in_card, text=self.get_text('in'), bg=self.colors['success'], 
                fg='white', font=('SF Pro Display', 14, 'bold')).pack(pady=(15, 5))
        self.in_label = tk.Label(in_card, text="0", bg=self.colors['success'], 
                                fg='white', font=('SF Pro Display', 42, 'bold'))
        self.in_label.pack()
        ...
        tk.Label(net_card, text="CURRENTLY INSIDE", bg=self.colors['accent'], 
                fg='white', font=('SF Pro Display', 12, 'bold')).pack(pady=(10, 2))
        self.net_label = tk.Label(net_card, text="0", bg=self.colors['accent'], 
                                 fg='white', font=('SF Pro Display', 32, 'bold'))
        self.net_label.pack()
    ...
    def create_button(self, parent, text, command, color):
        ...
        canvas = tk.Canvas(parent, width=120, height=50, bg='#1a1a2e',
                          highlightthickness=0, cursor='hand2')
        
        # Draw rounded rectangle
        rect = canvas.create_rectangle(5, 5, 115, 45, fill=color, outline='',
                                      width=0, tags='btn_bg')
        txt = canvas.create_text(60, 25, text=text, fill='white',
                                font=('Arial', 12, 'bold'), tags='btn_text')
    ...
    def lighten_color(self, color):
        """Lighten a hex color"""
        if color == "#22c55e": return "#4ade80"
        if color == "#f43f5e": return "#fb7185"
        if color == "#eab308": return "#fde047"
        return color
